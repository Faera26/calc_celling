...
import CeilingBuilderDialog from '../components/CeilingBuilderDialog';
...
function roomMetricValue(room: SavedEstimateRoom, metric: EstimateCalculationMetric) {
  if (metric === 'area') return num(room.area);
  if (metric === 'perimeter') return num(room.perimeter);
  if (metric === 'corners') return num(room.corners);
  if (metric === 'light_points') return num(room.light_points);
  if (metric === 'pipes') return num(room.pipes);
  if (metric === 'curtain_tracks') return num(room.curtain_tracks);
  if (metric === 'niches') return num(room.niches);
  return metric === 'fixed' ? 1 : 0;
}

function roundRuleQty(value: number, step: number) {
  if (!Number.isFinite(value) || value <= 0) return 0;
  const normalizedStep = num(step);
  return normalizedStep > 0
    ? Math.round(Math.ceil(value / normalizedStep) * normalizedStep * 100) / 100
    : Math.round(value * 100) / 100;
}

function applySketchMetricsToRoom(
  room: SavedEstimateRoom,
...
function autoRulesOf(estimate: SavedEstimate | undefined) {
  const snapshot = estimateSettingsWithRules(estimate) as EstimateSettingsSnapshot;
  return Array.isArray(snapshot.calculation_rules)
    ? snapshot.calculation_rules as EstimateCalculationRule[]
    : [];
}
...
export default function EstimatesPage({ auth, settings }: EstimatesPageProps) {
  ...
  function updateRoomSketch(roomId: string, sketch: CeilingSketch, metrics: CeilingSketchMetrics) {
    setRoomDrafts((prev) => prev.map((room) => (
      room.id === roomId ? applySketchMetricsToRoom(room, sketch, metrics) : room
    )));
  }

  function syncAutoPositionsForRoom(roomId: string, sketch: CeilingSketch, metrics: CeilingSketchMetrics) {
    const currentRoom = roomDrafts.find((room) => room.id === roomId);
    if (!currentRoom) return;

    const nextRoom = applySketchMetricsToRoom(currentRoom, sketch, metrics);
    const rulesById = new Map(autoRulesOf(selectedEstimate).map((rule) => [rule.id, rule]));

    setPositionDrafts((prev) => prev.map((position) => {
      if (position.room_id !== roomId || !isAutoGenerated(position)) return position;

      const source = sourceSnapshot(position);
      const rule = source.rule_id ? rulesById.get(String(source.rule_id)) : null;
      if (!rule) return position;

      const rawQty = rule.metric === 'fixed'
        ? rule.multiplier
        : roomMetricValue(nextRoom, rule.metric) * rule.multiplier;
      const qty = roundRuleQty(rawQty, rule.round_to);
      return recalculateSavedEstimatePosition(position, { qty });
    }));
  }
      ...
      {editingRoomSketch && (
        <CeilingBuilderDialog
          ...
          onApply={(sketch, metrics) => {
            ...
            syncAutoPositionsForRoom(editingRoomSketch.id, sketch, metrics);
            setEditingRoomSketchId(null);
          }}
        />
      )}
